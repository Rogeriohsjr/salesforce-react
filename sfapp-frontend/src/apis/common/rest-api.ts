...
export default class RestApi {
  ...
  public static delete<T>(pEndpoint: string, pSuccessCallBack: (response: T) => void, pErrorCallBack: (response: ISalesforceErrorDTO) => void): void {
    // We will call function
    fetch(pEndpoint, {
      method: 'delete',
    })
      .then((response) => {
        if (response.ok) {
          pSuccessCallBack(response as T);
        } else {
